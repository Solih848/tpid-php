...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Hilangkan simbol mata uang dan pemisah ribuan, lalu ubah ke angka
    $harga_kemarin = str_replace(['Rp', '.', ','], '', $_POST['harga_kemarin']);
    $harga_kemarin = (int) $harga_kemarin; // Konversi ke angka
    
    $harga_sekarang = str_replace(['Rp', '.', ','], '', $_POST['harga_sekarang']);
    $harga_sekarang = (int) $harga_sekarang;

    $query_update = "UPDATE sub_judul_produk SET judul_produk = :judul_produk, nama_bahan_pokok = :sub_judul_produk, nama_pasar = :nama_pasar, tanggal = :tanggal, satuan = :satuan, harga_kemarin = :harga_kemarin, harga_sekarang = :harga_sekarang WHERE id = :id";
    ...
    $stmt_update->bindParam(':harga_kemarin', $harga_kemarin); // Bind nilai yang sudah diperbaiki
    $stmt_update->bindParam(':harga_sekarang', $harga_sekarang);

    $stmt_update->execute(); // Eksekusi query update
...
<body>
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <label for="harga_kemarin">Harga Kemaren:</label>
            <input type="text" id="harga_kemarin" name="harga_kemarin" required readonly value="<?php echo isset($row['harga_kemarin']) ? 'Rp ' . number_format($row['harga_kemarin'], 0, ',', '.') : ''; ?>" style="width: calc(100% - 0px);">
